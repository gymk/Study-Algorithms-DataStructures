...
#define TILING_HEIGHT (4)
static int* gpi32TempResults;

int GetTiling(int width, int x, int y, int x_incr, int y_incr)
{
   int horz = 0;
   int vert = 0;
   ...
   if((x + 2) <= width)
   {
      horz = GetTiling(width + 2, x + 2, y);
   }
   if((y + 1) <= TILING_HEIGHT)
   {
      vert = GetTiling(width, x, y + 1);
   }
